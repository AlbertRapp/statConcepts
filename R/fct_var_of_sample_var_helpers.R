...
create_sidebar_panel <- function(id, max_samples, ns) {
  sidebarPanel(
    withMathJax(),
    # selectInput(
    ...
    fluidRow(
      column(
        ...
        splitLayout(
          ...
          actionButton(ns(glue("draw_sample_{id}")), "Draw!", width = '100%'),
          actionButton(ns(glue("prev_button_{id}")), "prev", width = '100%'),
          actionButton(ns(glue("next_button_{id}")), "next", width = '100%')
        )
      )
    )
  )
...
create_sidebar_layout <- function(id, max_samples, ns) {
  sidebarLayout(
    create_sidebar_panel(id, max_samples, ns),
    mainPanel(
      plotOutput(ns(glue("plot_{id}")))
    )
  )
}
